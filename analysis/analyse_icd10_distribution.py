...
def get_args():
    ap = argparse.ArgumentParser(description="Analyze ICD-10 code distribution in dataset")
    ...
    ap.add_argument("--icd10_pickle", default="MasterThesis/dataset/codes/icd10.pkl",
                    help="Path to ICD-10 code reference")
    ap.add_argument("--label_col", default="icd_code", help="Column containing ICD codes")
    ap.add_argument("--output_dir", default="icd10_analysis",
                    help="Directory to save analysis outputs")
    ap.add_argument("--top_n", type=int, default=20, 
                    help="Number of top codes to display")
    return ap.parse_args()

# ICD-10 chapter definitions based on provided categories
ICD10_CHAPTERS = {
...
def format_icd10_properly(code: str) -> str:
    """Format ICD-10 code to standard format."""
    ...
    # Basic validation - ICD-10 starts with a letter followed by numbers
...
def is_valid_icd10(code: str) -> bool:
    """Check if the code is a valid ICD-10 code."""
    if not code or not isinstance(code, str):
        return False
    
    # ICD-10 format: Letter followed by 2 digits, then optional .digit
    return bool(re.match(r'^[A-Z]\d\d(\.\d+)?$', code))

def get_icd10_parent(code: str) -> str:
    """Get the parent category of an ICD-10 code (first 3 characters)."""
    if not code or not isinstance(code, str):
        return ""
    
    # For ICD-10, the parent is typically the letter and first two digits
...
def load_and_prepare_data(args):
    ...
    if args.train_pickle and args.val_pickle and args.test_pickle:
        ...
        train_df = all_df  # For consistency
...
def extract_all_codes(df: pd.DataFrame, label_col: str) -> List[str]:
    """Extract all ICD-10 codes from the dataset with proper formatting."""
    all_codes = []
    for code_list in df[label_col]:
        formatted_codes = []
        for code in code_list:
            formatted = format_icd10_properly(str(code))
            if is_valid_icd10(formatted):
                formatted_codes.append(formatted)
        all_codes.extend(formatted_codes)
    return all_codes

def get_code_distribution(codes: List[str]) -> Dict[str, int]:
    """Count frequency of each ICD-10 code."""
...
def plot_code_frequency_histogram(code_counts: Dict[str, int], output_dir: str):
    ...
    plt.ylabel('Number of Unique ICD-10 Codes (log scale)')
    plt.title('Distribution of ICD-10 Code Frequencies')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "icd10_frequency_histogram.png"), dpi=300)
    plt.close()
    ...
    plt.title('Cumulative Distribution of ICD-10 Codes')
    plt.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "icd10_cumulative_distribution.png"), dpi=300)
    plt.close()

def plot_top_codes(code_counts: Dict[str, int], top_n: int, icd10_pickle: str, output_dir: str):
    """Plot the top N most frequent ICD-10 codes with descriptions."""
    os.makedirs(output_dir, exist_ok=True)
    
    # Try to load ICD-10 descriptions if available
    try:
        icd10_df = pd.read_pickle(icd10_pickle)
        code_to_desc = dict(zip(
            icd10_df['icd_code'].apply(format_icd10_properly),
            icd10_df['long_title']
        ))
    except Exception as e:
        print(f"Could not load ICD-10 descriptions: {e}")
        code_to_desc = {}
    ...
    plt.title(f'Top {top_n} Most Common ICD-10 Codes\n'
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"top_{top_n}_icd10_codes.png"), dpi=300)
    plt.close()
    ...
    top_df.to_csv(os.path.join(output_dir, f"top_{top_n}_icd10_codes.csv"), index=False)
...
def analyze_codes_per_visit(df: pd.DataFrame, label_col: str, output_dir: str):
    ...
    codes_per_visit = df[label_col].apply(lambda x: len([
        c for c in x if is_valid_icd10(format_icd10_properly(str(c)))
    ]))
    ...
    plt.title('Distribution of ICD-10 Codes per Patient Visit')
    ...
    plt.title('Number of ICD-10 Codes per Visit at Different Percentiles')
...
def get_icd10_chapter(code: str) -> str:
    """Return the official ICD-10-CM chapter for a given code."""
    if not code or not isinstance(code, str) or len(code) < 1:
        return "Unknown"
    
    # Extract first character (letter)
    first_char = code[0].upper()
    
    # Match to ICD-10 chapters based on the provided categories
    if first_char == 'A' or first_char == 'B':
        return "A00-B99: Certain infectious and parasitic diseases"
    elif first_char == 'C' or (first_char == 'D' and len(code) >= 3 and int(code[1:3]) <= 49):
        return "C00-D49: Neoplasms"
    elif first_char == 'D' and len(code) >= 3 and 50 <= int(code[1:3]) <= 89:
        return "D50-D89: Diseases of the blood and blood-forming organs"
    elif first_char == 'E':
        return "E00-E89: Endocrine, nutritional and metabolic diseases"
    elif first_char == 'F':
        return "F01-F99: Mental and Behavioral disorders"
    elif first_char == 'G':
        return "G00-G99: Diseases of the nervous system"
    elif first_char == 'H' and len(code) >= 3 and int(code[1:3]) <= 59:
        return "H00-H59: Diseases of the eye and adnexa"
    elif first_char == 'H' and len(code) >= 3 and int(code[1:3]) >= 60:
        ...
        return "K00-K95: Diseases of the digestive system"
    elif first_char == 'L':
        ...
        return "O00-O9A: Pregnancy, childbirth and the puerperium"
    elif first_char == 'P':
        return "P00-P96: Conditions originating in the perinatal period"
    elif first_char == 'Q':
        return "Q00-Q99: Congenital malformations and chromosomal abnormalities"
    elif first_char == 'R':
        return "R00-R99: Symptoms, signs and abnormal findings, NEC"
    elif first_char == 'S' or first_char == 'T':
        return "S00-T88: Injury, poisoning and external causes consequences"
    elif first_char == 'U':
        return "U00-U85: Codes for special purposes"
    elif first_char == 'V' or first_char == 'W' or first_char == 'X' or first_char == 'Y':
        return "V00-Y99: External causes of morbidity"
    elif first_char == 'Z':
...
def analyze_code_hierarchy(all_codes: List[str], output_dir: str):
    """Analyze the distribution of ICD-10 code categories/chapters."""
    os.makedirs(output_dir, exist_ok=True)
    
    # Get chapters for each code
    chapters = [get_icd10_chapter(code) for code in all_codes]
    chapter_counts = Counter(chapters)
    
    # Also get parent codes for more detailed analysis
    parent_codes = [get_icd10_parent(code) for code in all_codes]
    ...
    plt.xlabel('ICD-10-CM Chapters')
    plt.ylabel('Frequency')
    plt.title('Distribution of ICD-10 Codes by Official Chapter')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "icd10_chapters.png"), dpi=300)
    plt.close()
    
    # Create pie chart of chapter distribution
    ...
    plt.title('Distribution of ICD-10 Codes by Official Chapter')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "icd10_chapters_pie.png"), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.ylabel('Frequency')
    plt.title('Top 20 Most Common ICD-10 Parent Categories')
    ...
    try:
        ...
        for chapter in top_chapters:
            ...
            subcategories = Counter([get_icd10_parent(code) for code in chapter_codes])
...
def analyze_code_sparsity(all_codes: List[str], icd10_pickle: str, output_dir: str):
    """Analyze how many potential ICD-10 codes are actually used in the dataset."""
    os.makedirs(output_dir, exist_ok=True)
    
    # Load reference ICD-10 codes
    try:
        icd10_df = pd.read_pickle(icd10_pickle)
        all_possible_codes = set(icd10_df['icd_code'].apply(format_icd10_properly))
        all_possible_codes = {c for c in all_possible_codes if is_valid_icd10(c)}
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "icd10_coverage.png"), dpi=300)
        plt.close()
        ...
        plt.title('ICD-10 Code Coverage by Chapter')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "icd10_chapter_coverage.png"), dpi=300)
        plt.close()
...
def analyze_long_tail(code_counts: Dict[str, int], output_dir: str):
    """Analyze the long tail distribution of ICD-10 codes."""
    os.makedirs(output_dir, exist_ok=True)
    ...
    plt.title('Number of Most Frequent ICD-10 Codes Needed for Different Coverage Levels')
    ...
    plt.title('ICD-10 Code Frequency by Rank (Power Law Distribution)')
    ...
    plt.ylabel('Number of Codes')
    plt.title('Distribution of ICD-10 Codes by Frequency Range')
...
def main():
    ...
    print(f"Total ICD-10 code occurrences: {len(all_codes)}")
    ...
    plot_top_codes(code_counts, args.top_n, args.icd10_pickle, args.output_dir)
    analyze_codes_per_visit(all_df, args.label_col, args.output_dir)
    analyze_code_hierarchy(all_codes, args.output_dir)
    analyze_code_sparsity(all_codes, args.icd10_pickle, args.output_dir)
    ...
    print("\n===== ICD-10 Code Distribution Summary =====")
