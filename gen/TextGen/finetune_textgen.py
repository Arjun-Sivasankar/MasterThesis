# # -*- coding: utf-8 -*-
# # train_textgen_icd.py
# # ------------------------------------------------------------------
# # Supervised fine-tuning (LoRA) for free-text diagnosis generation,
# # then mapping generated terms -> ICD-9 via SapBERT+FAISS+fuzzy.
# #
# # Index folder layout (icd_index_dir):
# #   - code2title.json : { "250.00": "Diabetes mellitus...", ... }
# #   - icd.faiss       : FAISS index built on texts in rows.json (same order)
# #   - rows.json       : [ {"text": "<title/synonym>", "code": "250.00"}, ... ]
# #   - meta.json       : { "encoder_model": "cambridgeltl/SapBERT-from-PubMedBERT-fulltext",
# #                         "metric": "ip" | "l2", "normalized": true }
# #   - embeddings.npy  : (optional; not used at runtime)
# # ------------------------------------------------------------------
...
#                  label_col: str,
#                  target_mode: str,            # "icd_titles" or "discharge_dx"
#                  icd_index_dir: str,
#                  max_len: int,
#                  N_max_terms: int):
#         self.tok = tokenizer
...
#     # -------- load data --------
#     if is_main_process(): log.info(f"Loading data: {args.data_pickle}")
#     try:
...
#     trainer.train()
#     if is_main_process():
#         log.info("Training done in {:.1f} min".format((time.time()-t0)/60.0))

#     # -------- test (rank 0 only) --------
#     if is_main_process():
#         log.info("TEST: generating diagnosis terms...")
#         test_prompts = [build_textgen_prompt(r, args.N_max_terms) for _, r in test_df.iterrows()]
#         terms_test = generate_terms(trainer.model, tok, test_prompts,
#                                     max_len=args.max_len, max_new=args.gen_max_new,
#                                     batch_size=max(1, args.per_device_eval_batch_size))
#         log.info("TEST: mapping to ICD...")
...
#         with open("runs_textgen/test_metrics.json","w") as f:
...
class SFTTextGenDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {"input_ids": input_ids, "attention_mask": attn, "labels": labels,
                "prompt_len": prompt_len, "idx": self.kept_idx[idx],
...
# ------------------ logging callback ------------------
# --- replace your LoggingCallback with this ---
import math
...
class BetterLoggingCallback(TrainerCallback):
    def __init__(self):
        ...
        self.step_losses_by_epoch = defaultdict(list)  # epoch -> [loss,...]
        self.eval_loss_by_epoch = {}                   # epoch -> eval_loss
        self.last_lr = None
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        # Called frequently during training/eval; only rank 0 logs
        if not is_main_process() or not logs:
            return
        # Track training step loss and LR
        if "loss" in logs:
            # logs["epoch"] is a float like 1.23; map to the current epoch index
    ...
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        # This is where eval_loss is reliably available
...
def log_test_results(results, labels_full, labels_head=None, eval_head_k=0):
    """Log test evaluation results in a readable format"""
...
def main():
    ...
    if is_main_process():
        ...
        log.info(f"Example input (tokenized, prompt + target): \n{train_ds[0]['input_ids']}\n")
        log.info(f"Example target (tokenized, labels with -100 for prompt): \n{train_ds[0]['labels']}\n")
        log.info(f'User message: {train_ds.__getitem__(0)["user_msg"]}')
        log.info(f'Assistant message: {train_ds.__getitem__(0)["asst_msg"]}')
    ...
    TA = TrainingArguments(
        ...
        # use the correct argument names & enable step logging
        logging_strategy="steps",
        logging_steps=50,  # or whatever cadence you like
        ...
        ddp_find_unused_parameters=False,
        local_rank=args.local_rank,
        disable_tqdm=True,  # keep it if you prefer clean logs
    )
    ...
    if is_main_process():
        ...
        # Generate terms (with progress reporting)
        ...
        for i in tqdm(range(0, len(test_prompts), bs), desc="Generating", disable=not is_main_process()):
            batch = test_prompts[i:i+bs]
            batch_results = generate_terms(
                trainer.model, tok, batch,
                max_len=args.max_len, max_new=args.gen_max_new, 
                batch_size=bs
            )
        ...
        if terms_test:
            ...
            for i in range(min(3, len(terms_test))):
                ...
                log.info(f"  Sample {i+1}: {', '.join(terms_to_show)}" + 
                         ("..." if len(terms_test[i]) > 3 else ""))
        
        # Mapping to ICD codes
        ...
        if mapped_test:
            ...
            for i in range(min(3, len(mapped_test))):
                ...
                log.info(f"  Sample {i+1}: {', '.join(codes_to_show)}" + 
                         ("..." if len(mapped_test[i]) > 5 else ""))
