# # gen/withKG/RAG/textgen_prompt_hpaths_ablations.py
# #!/usr/bin/env python3
# # -*- coding: utf-8 -*-
...
# - Logs RAW and KG free-text terms for sample rows.

# NEW (ablation):
# - --kg_block {both,h1,h2} to run RAW vs H1-only, H2-only, or H1+H2 (ratio-controlled).

# NEW (debug):
# - Prints the FIRST sample's full prompts (RAW & KG) and token counts during prompt build.
# - Prints the FIRST sample's full RAW & KG generations after merge at the end.
# """
...
#                     "rel_uv": (d_uv.get("rel") or ""),  
#                     "rela_uv": (d_uv.get("rela") or ""),
#                     "rel_vw": (d_vw.get("rel") or ""),  
#                     "rela_vw": (d_vw.get("rela") or ""),
#                 })
#     return H1, H2
...
#     mode:
#       - "both": split budget per h2_ratio (default)
#       - "h1"  : include only H1 (budget all to H1)
#       - "h2"  : include only H2 (budget all to H2)

#     If budget <= 0: unlimited for the selected mode.
#     """
...
#         if mode == "h1":
#             combined = h1_text
#             return combined, count_tokens(tok, combined), 0, count_tokens(tok, h1_text)
#         if mode == "h2":
#             combined = h2_text
#             return combined, count_tokens(tok, combined), count_tokens(tok, h2_text), 0
#         combined = h2_text + ("\n" + h1_text if h1_text else "")
#         return combined, count_tokens(tok, combined), count_tokens(tok, h2_text), count_tokens(tok, h1_text)

#     if mode == "h1":
#         h1_trim = trim_to_token_budget(tok, h1_text, budget)
#         return h1_trim, count_tokens(tok, h1_trim), 0, count_tokens(tok, h1_trim)

#     if mode == "h2":
#         h2_trim = trim_to_token_budget(tok, h2_text, budget)
#         return h2_trim, count_tokens(tok, h2_trim), count_tokens(tok, h2_trim), 0
...
#     # roll leftover from H2 to H1 once
...
#                     help="Ablation: include H1 only, H2 only, or both (ratio-controlled).")
...
#     ap.add_argument("--max_neighbors_show", type=int, default=24)  # compatibility
#     # simple miner caps
...
#                     # set split by mode
#                     if args.kg_block == "h1":
#                         kg_h2_tokens, kg_h1_tokens = 0, kg_total_tokens
#                     elif args.kg_block == "h2":
#                         kg_h2_tokens, kg_h1_tokens = kg_total_tokens, 0
#                     else:
...
#         # ---------- FIRST SAMPLE DEBUG: PROMPTS & TOKENS ----------
#         if i == 0 and is_main_process():
#             print("\n" + "="*80)
#             print("FIRST SAMPLE DEBUG — PROMPTS / TOKENS")
#             print("="*80)
#             print(f"hadm_id: {row.get('hadm_id','')}")
#             print(f"kg_block_mode: {getattr(args, 'kg_block', 'both')}")
#             print(f"N_cap: {N_this}")
#             print(f"header_tokens: {d['header_tokens']}")
#             print(f"notes_tokens : {d['notes_tokens']}")
#             print(f"tail_tokens  : {d['tail_tokens']}")
#             print(f"RAW_total    : {d['total_raw']}")
#             print(f"WITHKG_total : {d['total_kg']}")
#             print(f"KG_total     : {kg_total_tokens}")
#             print(f"KG_H2_tokens : {kg_h2_tokens}")
#             print(f"KG_H1_tokens : {kg_h1_tokens}")
...
#         all_dbg = []
#         all_rawgen, all_kggen = [], []   # <-- fixed name
...
#             all_rawgen.extend(D["raw_texts"])
#             all_kggen.extend(D["kg_texts"])   # <-- fixed name
...
#         rawgen_all    = [all_rawgen[i] for i in order]
#         kggen_all     = [all_kggen[i]  for i in order]   # <-- fixed name

#         # -------- FIRST SAMPLE FULL GENERATIONS (RAW & KG) --------
#         if len(rawgen_all) > 0:
#             print("\n" + "="*80)
#             print("FIRST SAMPLE DEBUG — FULL GENERATIONS")
#             print("="*80)
#             try:
#                 print("\n---- RAW GENERATION (Sample 1) ----")
#                 print(_coerce_text(rawgen_all[0]))
#             except Exception as e:
#                 print(f"(raw generation print error: {e})")
#             try:
#                 print("\n---- KG  GENERATION (Sample 1) ----")
#                 print(_coerce_text(kggen_all[0]))   # <-- fixed name
#             except Exception as e:
#                 print(f"(kg generation print error: {e})")
#             print("="*80 + "\n")
...
#         # print a few side-by-side samples (includes RAW/KG free-text terms)
...
UPDATED:
- Use rela_canon (and NOT rela) for path labels that go into prompts.
- Score-aware ordering by rela_score (H1 by edge score; H2 by sum of two edges).
- For the FIRST sample print, append the scores inline next to each printed path
  (purely display; not counted in tokens and not included in kg_text_combined).
"""
...
def extract_after_output(generation, n_max: int = 12) -> List[str]:
    ...
    for raw in block.splitlines():
        ...
        if not line:
            continue
        if line.startswith("[]") or (line.startswith("[") and not re.match(r"^\[\s*OUTPUT\s*\]$", line, flags=re.I)):
            break
        line = re.sub(r"^(?:[-*]\s*|\d+\.\s*|\(\d+\)\s*)", "", line)
        if line:
            lines_out.append(line)
...
def mine_hops_simple(G: nx.DiGraph,
    ...
    """
    Score-aware miner:
      H1: u -> v  (sorted by edge['rela_score'] desc)
      H2: u -> v -> w (sorted by sum of two edges' rela_score desc)
    """
    H1, H2 = [], []

    def _edge_attrs(d):
        # canon rel + score, w/ safe fallbacks
    ...
    for u in ev_cuis:
        ...
        # First hop: collect and sort by score desc
        first = []
        for v in G.successors(u):
            d = G[u][v]
            ...
            first.append((
                v,
                rela_canon,
                -1e9 if score is None else score,  # None -> very low
                G.nodes[v].get("name","")
            ))
        ...
        # H2 rows (sum of two edges' scores)
        second = []
        for v, rela_uv_canon, score_uv, vname in first:
            ...
            for w in G.successors(v):
                ...
                second.append((
                    total, v, w,
                    rela_uv_c, (None if s_uv == -1e9 else s_uv),
                    rela_vw_c, (None if s_vw == -1e9 else s_vw),
                    vname, G.nodes[w].get("name","")
                ))
        ...
        for total, v, w, rela_uv_c, s_uv, rela_vw_c, s_vw, vname, wname in second:
            H2.append({
                ...
                # canon + scores (used for ordering and debug display)
                "rela_uv_canon": rela_uv_c,
                "rela_vw_canon": rela_vw_c,
                "score_uv": s_uv,
                "score_vw": s_vw,
                "score_total": ((s_uv or 0.0) + (s_vw or 0.0)),
            })

    return H1, H2

def render_h2_block(H2_rows: List[dict]) -> str:
    # Canon labels ONLY (no scores) -> prompt-safe
    lines = ["[KG context - H2 paths]"]
    for c in H2_rows:
        u = c.get("u_name") or c.get("u") or ""
        v = c.get("v_name") or c.get("v") or ""
...
def main():
    ...
    if is_main_process():
        log.info(f"KG H2 Ratio: {args.kg_h2_ratio}")
        log.info(f"Test size: {len(test_df)}")
        log.info(f"Eval label space: {len(labels_eval)} codes ({'FULL' if head_name is None else head_name})")
    ...
    # ---------------- KG + maps ----------------
    KG  = pickle.load(open(args.kg_pkl, "rb"))  # expected to be nx.DiGraph with rela_canon, rela_score on edges
    icd9_dx_map   = pickle.load(open(args.icd9_dx_map_pkl,   "rb"))
    icd9_proc_map = pickle.load(open(args.icd9_proc_map_pkl, "rb"))
    loinc_map     = pickle.load(open(args.loinc_map_pkl,     "rb"))
    atc_map       = pickle.load(open(args.atc_map_pkl,       "rb"))
    ...
    for i, row in tqdm(test_df.iterrows(), total=len(test_df)):
        ...
        # Mine & render H2/H1 path blocks (score-aware, canon rels)
        H1_rows, H2_rows = mine_hops_simple(KG, ev_cuis, k1=args.kg_k1, k2=args.kg_k2)
        h2_block = render_h2_block(H2_rows) if H2_rows else "[KG context - H2 paths]\n- (none)"
        h1_block = render_h1_block(H1_rows) if H1_rows else "[KG context - H1 paths]\n- (none)"
        kg_text_combined, kg_total_tokens, kg_h2_tokens, kg_h1_tokens = combine_kg_blocks_with_budget(
        ...
        if i == 0 and is_main_process():
            ...
            # Same paths printed with scores appended (display only; not part of prompt)
            try:
                print("\n---- KG BLOCK (COMBINED with scores — display only; NOT counted) ----")
                # H2 with scores
                if H2_rows:
                    print("[KG context - H2 paths]")
                    for c in H2_rows:
                        u = c.get("u_name") or c.get("u") or ""
                        v = c.get("v_name") or c.get("v") or ""
                        w = c.get("w_name") or c.get("w") or ""
                        ...
                        s_uv = c.get("score_uv")
                        s_vw = c.get("score_vw")
                        s_tot = c.get("score_total")
                        print(f"- {u}{_arrow_label(r_uv, '')}{v}{_arrow_label(r_vw, '')}{w}  "
                              f"(scores: uv={s_uv if s_uv is not None else 'NA'}, "
                              f"vw={s_vw if s_vw is not None else 'NA'}, "
                              f"total={s_tot if s_tot is not None else 'NA'})")
                else:
                    print("[KG context - H2 paths]\n- (none)")
                # H1 with scores
                if H1_rows:
                    print("[KG context - H1 paths]")
                    for c in H1_rows:
                        u = c.get("src_name") or c.get("src_cui") or ""
                        v = c.get("nbr_name") or c.get("nbr_cui") or ""
                        r = c.get("rela_canon") or c.get("rela") or c.get("rel") or ""
                        s = c.get("rela_score")
                        print(f"- {u}{_arrow_label(r, '')}{v}  (score={s if s is not None else 'NA'})")
                else:
                    print("[KG context - H1 paths]\n- (none)")
            except Exception as e:
                print(f"(could not render scored KG display: {e})")
    ...
    if rank == 0:
        ...
        if len(rawgen_all) > 0:
            ...
            try:
                print("\n---- KG  GENERATION (Sample 1) ----")
                print(_coerce_text(kggen_all[0]))
            except Exception as e:
                print(f"(kg generation print error: {e})")
            print("="*80 + "\n")
