...
def save_separate_indexes(
    ...
    h2_first_counter = Counter(h2_first_rels)
    h2_second_counter = Counter(h2_second_rels)
    
    log.info(f"  First relationship distribution (top 10):")
    for rel, count in h2_first_counter.most_common(10):
        log.info(f"    - {rel}: {count}")
    
    log.info(f"  Second relationship distribution (top 10):")
    for rel, count in h2_second_counter.most_common(10):
        log.info(f"    - {rel}: {count}")
    ...
    meta = {
        ...
        "h2_index": {
            ...
            "second_relationship_distribution": dict(h2_second_counter.most_common(20)),
