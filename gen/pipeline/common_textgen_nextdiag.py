...
def load_kg_mappings(kg_path: str) -> Dict[str, str]:
    """
    Load knowledge graph mappings from kg_nodes.csv
    Returns dict: {code: description}
    """
    if not kg_path:  # Handle None or empty string
        return {}
        
    if not os.path.exists(kg_path):
        if is_main_process():
            log.warning(f"KG file not found: {kg_path}")
        return {}
    ...
    try:
        ...
        for _, row in kg_df.iterrows():
            ...
            if code and desc and len(desc) > 3:
                code2desc[code.upper()] = desc
        
        if is_main_process():
            log.info(f"✓ Loaded {len(code2desc)} KG mappings from {kg_path}")
        
        return code2desc
    except Exception as e:
        if is_main_process():
            log.warning(f"Error loading KG mappings: {e}")
        return {}

def map_codes_to_descriptions(codes: List[str], kg_map: Dict[str, str], 
                              max_codes: int = 24) -> str:
    """
    Map codes to their descriptions using KG mappings.
    If kg_map is empty, returns space-separated codes (original behavior).
    """
    if not codes:
        return ""
    
    # If no KG mappings, return original format
    if not kg_map:
    ...
    for code in codes[:max_codes]:
        ...
        desc = kg_map.get(code_upper, "")
        if desc:
            mapped.append(f"{code}:{desc}")
        else:
            mapped.append(code)  # Fallback to code if no mapping
    
    return " | ".join(mapped)

# Update serialize_structured_readable - ADD kg_map parameter with default None
def serialize_structured_readable(row: pd.Series, include_index_icd: bool = False,
                                 kg_map: Dict[str, str] = None) -> str:
    """
    Enhanced version with optional KG mappings for structured codes.
    Falls back to original behavior if kg_map is None or empty.
    """
    kg_map = kg_map or {}  # Handle None
    
    parts = []
    parts.append(f"DEMOGRAPHICS: gender={row.get('gender','')} age_group={row.get('age','')}")
    
    # INDEX DIAGNOSES with optional descriptions
    if include_index_icd:
        idx_codes = [format_icd9(c) for c in to_list(row.get("icd_code", [])) if c]
        idx_codes = [c for c in idx_codes if is_valid_icd9(c)]
        if idx_codes:
            mapped_idx = map_codes_to_descriptions(idx_codes, kg_map, max_codes=32)
            parts.append(f"INDEX DIAGNOSES: {mapped_idx}")
    
    # MEDICATIONS with optional descriptions
    ndc_codes = to_list(row.get("ndc", []))
    if ndc_codes:
        mapped_ndc = map_codes_to_descriptions(ndc_codes, kg_map, max_codes=24)
        parts.append(f"MEDICATIONS: {mapped_ndc}")
    
    # PROCEDURES with optional descriptions
    proc_codes = to_list(row.get("pro_code", []))
    proc_codes = [format_icd9_proc_from_pro(c) for c in proc_codes]
    if proc_codes:
        mapped_proc = map_codes_to_descriptions(proc_codes, kg_map, max_codes=24)
        parts.append(f"PROCEDURES: {mapped_proc}")
    
    # LAB TESTS with optional descriptions
    lab_codes = to_list(row.get("lab_test_loinc", []))
    if lab_codes:
        mapped_labs = map_codes_to_descriptions(lab_codes, kg_map, max_codes=48)
        parts.append(f"LAB TESTS: {mapped_labs}")
    
    return "\n".join(parts)

# def serialize_structured_readable(row: pd.Series, include_index_icd: bool = False) -> str:
...
#         idx_codes = [c for c in idx_codes if is_valid_icd9(c)]
#         if idx_codes:
#             parts.append("INDEX DIAGNOSES: " + " ".join(idx_codes[:32]))
...
def build_textgen_prompt_budgeted(row: pd.Series, tok, max_len: int,
    ...
    """
    Build prompt with budget management and optional KG-enhanced descriptions.
    """
    head = []
    head.append(f"[VISIT] subject_id={row.get('subject_id_x','?')} hadm_id={row.get('hadm_id','?')}")
    head.append(serialize_structured_readable(row, include_index_icd=include_index_icd, kg_map=kg_map))
    notes_full = serialize_notes(row)

    # Compute adaptive target if enabled
    if adaptive_max_terms and include_index_icd:
        current_codes = to_list(row.get('icd_code', []))
        n_current = len(current_codes)
        
        # Window-specific multipliers based on analysis
        if future_window == "6M":
            # 6M: ratio=1.14x, use 1.15 for safety
            expected_target = max(10, int(n_current * 1.15))
        elif future_window == "12M":
            # 12M: ratio=1.00x, use 1.0
            expected_target = max(8, int(n_current * 1.0))
        else:
            expected_target = max(10, int(n_current * 1.1))
        
        # Cap at N_max_terms
        expected_target = min(expected_target, N_max_terms)
    else:
        # Static mode: use median from analysis
        if future_window == "6M":
            expected_target = 14  # median from analysis
        elif future_window == "12M":
            expected_target = 12  # median from analysis
        else:
            expected_target = 12

    # Window-specific guidance
    if future_window == "6M":
        task_desc = "[TASK] Based on this admission, predict all the likely diagnoses to occur in the next 6 months."
        if adaptive_max_terms:
            guidance = f"Expected: ~{expected_target} diagnoses. Include both persistent and emergent conditions."
        else:
            guidance = "Expected: 10-19 diagnoses (typically ~14). Include both persistent and emergent conditions."
    elif future_window == "12M":
        task_desc = "[TASK] Based on this admission, predict all the likely diagnoses to occur in the next 12 months."
        if adaptive_max_terms:
            guidance = f"Expected: ~{expected_target} diagnoses. Include both persistent and emergent conditions."
        else:
            guidance = "Expected: 8-18 diagnoses (typically ~12). Include both persistent and emergent conditions."
    else:
        task_desc = f"[TASK] Predict future diagnoses in the next {future_window}."
        guidance = f"Expected: ~{expected_target} diagnoses."
    ...
    tail = [
        ...
        f"- Target: around {expected_target} lines (maximum: {N_max_terms})",
        "[OUTPUT]",
    ]
...
class SFTTextGenDataset(Dataset):
    def __init__(self, df: pd.DataFrame, tokenizer,
                 ...
                 new_only: bool = False,
                 adaptive_max_terms: bool = False,
                 kg_path: str = None):  # NEW PARAMETER with default None

        self.kg_path = load_kg_mappings(kg_path) if kg_path else {}
        self.tok = tokenizer
        ...
        self.include_index_icd = include_index_icd
        self.new_only = new_only
        self.adaptive_max_terms = adaptive_max_terms
        ...
        for idx, row in df.reset_index(drop=True).iterrows():
            if target_mode == "icd_titles":
                next_codes = [format_icd9(c) for c in to_list(row.get(label_col, [])) if c]
                next_codes = [c for c in next_codes if is_valid_icd9(c)]

                if self.new_only:
                    idx_codes = {format_icd9(c) for c in to_list(row.get("icd_code", [])) if is_valid_icd9(format_icd9(c))}
                    next_codes = [c for c in next_codes if c not in idx_codes]
            ...
            prompt, stat = build_textgen_prompt_budgeted(
                ...
                include_index_icd=self.include_index_icd,
                adaptive_max_terms=self.adaptive_max_terms,
                kg_map=self.kg_path # Pass the loaded kg_map (may be empty)
            )

            if idx==0 and is_main_process():
                log.info(f"Sample prompt (idx={idx}):\n{prompt}\n---\nTarget:\n{target}\n===")
        ...
        if is_main_process():
            mode_str = "ADAPTIVE" if self.adaptive_max_terms else "STATIC"
            log.info(
                ...
                f"(mode={target_mode}, window={future_window}, "
                f"new_only={self.new_only}, include_index_icd={self.include_index_icd}, "
                f"max_terms_mode={mode_str})"
            )
