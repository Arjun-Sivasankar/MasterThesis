...
# Constants
UMLS_KB_PATH = "/Users/arjun/Desktop/Thesis/UMLS/2025AA/META"
...
def generate_mappings(method: str) -> Tuple[Dict[str, List[str]], Dict[str, List[str]]]:
    ...
    cui_to_icd9 = defaultdict(list)
    cui_to_icd10pcs = defaultdict(list)
    ...
    with open(MRCONSO_FILE, "r", encoding="utf-8") as f:
        ...
        for row in tqdm(reader):
            ...
            if sab == "SNOMEDCT_US":
                ...
                if code not in cui_to_icd9[cui]:
                    cui_to_icd9[cui].append(code)
            elif sab == "ICD10PCS":
                if code not in cui_to_icd10pcs[cui]:
                    cui_to_icd10pcs[cui].append(code)
            elif sab == "ICD10CM":
    ...
    return dict(cui_to_snomed), dict(cui_to_icd10), dict(cui_to_icd9), dict(cui_to_icd10pcs)
...
def save_mappings(method: str):
    ...
    cui_to_snomed_list, cui_to_icd10_list, cui_to_icd9_list, cui_to_icd10pcs_list = generate_mappings(method)
    map_file_path = os.path.join(os.path.dirname(__file__), "mappings")
    if not os.path.exists(map_file_path):
        os.makedirs(map_file_path)

    snomed_filename = os.path.join(map_file_path, f"cui_to_snomed_{method}.json")
    icd10_filename = os.path.join(map_file_path, f"cui_to_icd10_{method}.json")
    icd9_filename = os.path.join(map_file_path, f"cui_to_icd9_{method}.json")
    icd10pcs_filename = os.path.join(map_file_path, f"cui_to_icd10pcs_{method}.json")

    with open(snomed_filename, "w") as snomed_file, open(icd10_filename, "w") as icd10_file, open(icd9_filename, "w") as icd9_file, open(icd10pcs_filename, "w") as icd10pcs_file:
        json.dump(cui_to_snomed_list, snomed_file)
        json.dump(cui_to_icd10_list, icd10_file)
        json.dump(cui_to_icd9_list, icd9_file)
        json.dump(cui_to_icd10pcs_list, icd10pcs_file)

    print(f"Mappings saved to {snomed_filename} and {icd10_filename}")
    print(f"Number of SNOMED CT mappings: {len(cui_to_snomed_list)}")
    print(f"Number of ICD-10 mappings: {len(cui_to_icd10_list)}")
    print(f"Number of ICD-9 mappings: {len(cui_to_icd9_list)}")
    print(f"Number of ICD-10PCS mappings: {len(cui_to_icd10pcs_list)}")
