...
#SBATCH --output=logs/preprocess/history_aware_%j.out # stdout+stderr log
...
echo "[INFO] History-Aware Dataset Creation for MIMIC-IV ..."
start_icd9=$(date +%s)

python -u dataset/history_aware_dataset.py \

end_icd9=$(date +%s)
echo "[TIME] History-Aware Dataset Creation completed in $((end_icd9-start_icd9)) seconds"
