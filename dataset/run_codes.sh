...
#SBATCH --time=4:00:00                # Max walltime
#SBATCH --output=logs/preprocess/codes_%j.out # stdout+stderr log
...
echo "[INFO] Codes preprocessing ..."
start_icd9=$(date +%s)

python -u dataset/codes.py \

end_icd9=$(date +%s)
echo "[TIME] Codes preprocessing completed in $((end_icd9-start_icd9)) seconds"
